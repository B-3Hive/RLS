...
-- 
-- Create Logins in Server using SEC_ORG_USER_BASE table
-- 
/*
DECLARE user_cursor CURSOR
   FOR
   SELECT userid from [RLS_2022].dbo.[SEC_ORG_USER_BASE];
OPEN user_cursor;
...
BEGIN;
   ...
   EXECUTE ('create login [' + @username + '] WITH PASSWORD=N''oilgaspass123!'', DEFAULT_DATABASE=[RLS_2022], CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF;');
   FETCH NEXT FROM user_cursor INTO @username;
END;
...
-- 
-- Create Users in Database using SEC_ORG_USER_BASE table (Skip/Comment Out Section above and run only this section for Azure SQL Database)
-- 
/*
DECLARE user_cursor CURSOR
   FOR
...
*/
 ...
 --* Use if just want users in DB and no logins with passwords for simple testing (comment out the I commented 2 blocks above) ****
...
create SECURITY POLICY [rls].[WELL_SecPol] 
  ...
  ADD BLOCK PREDICATE [rls].[fn_WELLMaster_SecBlockPred]() ON [dbo].[WELL_MASTER] AFTER INSERT,
  ADD BLOCK PREDICATE [rls].[fn_WELL_SecPred]([WELL_ID]) ON [dbo].[WELL_MASTER] AFTER UPDATE,
